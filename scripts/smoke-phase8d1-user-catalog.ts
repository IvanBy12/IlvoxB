...
const ids = {
  ...
  client: randomUUID(), noneligible: randomUUID(), scoped: randomUUID(),
...
async function cleanup() {
  ...
  try {
    const userIds = [ids.owner, ids.pending, ids.blocked, ids.deleted, ids.client, ids.noneligible, ids.scoped];
    ...
    await pool.query("DELETE FROM project_members WHERE project_id = ANY($1::uuid[])", [[ids.projectA, ids.projectB]]);
    await pool.query("DELETE FROM projects WHERE id = ANY($1::uuid[])", [[ids.projectA, ids.projectB]]);
...
try {
  ...
  for (const [label, items] of Object.entries({ accountManagers, projectLeads, projectMembers, taskAssignees, ticketAssignees, leadAssignees })) {
